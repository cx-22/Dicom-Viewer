...
void QRangeSlider::setMinimum(const int minimum)
{
...
void QRangeSlider::setMaximum(const int maximum)
{
...
int QRangeSlider::lowValue() const
{
    return m_lowValue;
}

void QRangeSlider::setLowValue(const int lowValue)
{
...
int QRangeSlider::highValue() const
{
    return m_highValue;
}

void QRangeSlider::setHighValue(const int highValue)
{
...
void QRangeSlider::setRange(const int minimum, const int maximum)
{
    setMinimum(minimum);
    setMaximum(maximum);
}
...
void QRangeSlider::mousePressEvent(QMouseEvent *e)
{
    if (e->position().y() >= (height() - SLIDER_HEIGHT - HANDLE_SIZE) / 2 && e->position().y() <= (height() - SLIDER_HEIGHT + HANDLE_SIZE) / 2) // Check if event was on slider
    {
        ...
        int mouseValue = (mouseX / width()) * (m_maximum - m_minimum) + m_minimum;
...
void QRangeSlider::mouseMoveEvent(QMouseEvent *e)
{
    if (m_lastMouseValue != -1)
    {
        ...
        if (m_lastMouseValue >= m_lowValue - normalizedHandleSize && m_lastMouseValue < m_lowValue + normalizedHandleSize)
        {
